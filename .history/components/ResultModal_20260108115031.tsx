...
export default function ResultModal({ data, onClose }: ResultModalProps) {
  ...
  const getSuggestedASN = (irrString: string) => {
    ...
    // Ambil object pertama, lalu pisahkan @APNIC dsb
    return irrString.split(' | ')[0].split('@')[0];
  };

  // 2. Tentukan Max Length (LOGIKA BARU: Hostmaster Best Practice)
  const getMaxLength = (cidr: string) => {
    // Cek apakah ini IPv6 (ada titik dua)
    if (cidr.includes(":")) {
      return "48"; // IPv6 standar de-agregasi biasanya /48
    }

    // Untuk IPv4, kita kunci di /24
    // Biar member bisa mecah blok besar (misal /23) jadi /24 tanpa invalid RPKI.
    return "24";
  };
