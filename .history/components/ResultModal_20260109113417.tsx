    ...
    <div className="fixed inset-0 bg-slate-900/80 z-50 flex items-end md:items-center justify-center md:p-4 backdrop-blur-sm animate-in fade-in duration-200">
      ...
      <div className="bg-white w-full md:rounded-2xl md:max-w-5xl h-[95vh] md:h-auto md:max-h-[90vh] flex flex-col border border-slate-200 shadow-2xl overflow-hidden rounded-t-2xl">
        ...
        <div className="px-6 py-4 md:py-6 border-b border-slate-100 bg-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4 z-10 shrink-0">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <button 
              ...
              className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-sm font-bold shadow-sm transition-all active:scale-95"
            >
            ...
            <button 
              onClick={onClose} 
              className="flex-none bg-slate-100 hover:bg-red-50 p-2 rounded-xl border border-slate-200 hover:border-red-200 transition-all text-slate-500 hover:text-red-500"
            >
            ...
            <div key={idx} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden group">
              ...
              <div className="p-4 md:p-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                  ...
                  <div className="space-y-4 md:space-y-5">
                    ...
                    <div className="pl-3 border-l-2 border-blue-200 space-y-1">
                      ...
                      {/* === UPDATE: PARENT DESCRIPTION DISINI === */}
                      <p className="text-xs text-slate-500 italic mb-1 border-l-2 border-slate-200 pl-2 py-0.5 bg-slate-50 rounded-r">
                        {item.parent_desc}
                      </p>
                       ...
                       {item.children === "-" ? <p className="text-sm text-slate-400 italic">No sub-allocations</p> : 
                        <div className="flex flex-col gap-2 mt-2">
                          {item.children.split(' | ').map((child, cIdx) => (
                            <div key={cIdx} className="px-3 py-2 bg-yellow-50 text-yellow-800 border border-yellow-100 rounded text-xs font-mono break-words leading-relaxed">
                              ↳ {child}
                            </div>
                          ))}
                        </div>
                ...
                {!item.rpki_status.includes("VALID") && (
                  <div className="mt-6 md:mt-8 relative group/terminal">
                    <div className="bg-slate-900 rounded-xl overflow-hidden shadow-lg border border-slate-700">
                      ...
                      <div className="p-4 md:p-5 font-mono text-sm overflow-x-auto">
                        <div className="flex items-start gap-3 mb-4 min-w-[300px]">
                          ...
                          <div>
                            <h5 className="text-slate-100 font-bold text-sm">Action Required: Create ROA</h5>
                            <p className="text-slate-400 text-xs mt-1">Status IP Invalid. Buat ROA di MyAPNIC.</p>
                          </div>
                        </div>
                        <div className="grid grid-cols-3 gap-4 border-t border-slate-700 pt-4 min-w-[300px]">
                           <div>
                             <p className="text-slate-500 text-[10px] uppercase">Origin ASN</p>
                             <p className="text-green-400 font-bold mt-1 text-xs md:text-sm">{getSuggestedASN(item.irr_objects)}</p>
                           </div>
                           <div>
                             <p className="text-slate-500 text-[10px] uppercase">Prefix</p>
                             <p className="text-blue-400 font-bold mt-1 text-xs md:text-sm">{item.cidr}</p>
                           </div>
                           <div>
                             <p className="text-slate-500 text-[10px] uppercase">Max Length</p>
                             <p className="text-purple-400 font-bold mt-1 text-xs md:text-sm">/{getMaxLength(item.cidr)}</p>
                           </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="p-4 border-t border-slate-100 bg-white z-10 flex justify-end shrink-0">
          <button 
            onClick={onClose} 
            className="w-full md:w-auto px-8 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-sm font-bold shadow-lg shadow-slate-300/50 transition-all active:scale-95"
          >
            Selesai
          </button>
        </div>
