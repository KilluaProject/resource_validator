...
export default function ASNResultModal({ data, onClose }: ASNResultModalProps) {
  ...
  // === LOGIC PECAH STRING HOLDER (Baru) ===
  // Misal: "IDNIC-XYZ-ID PT Mencari Cinta Sejati"
  // Split spasi pertama: 
  // netname = "IDNIC-XYZ-ID"
  // company = "PT Mencari Cinta Sejati"
  const splitHolder = (fullString: string) => {
    const parts = fullString.split(' ');
    const netname = parts[0]; // Kata pertama
    const company = parts.slice(1).join(' '); // Sisa kalimat
    return { netname, company };
  };

  const { netname, company } = splitHolder(data.holder);
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex items-start gap-4">
                ...
                <div>
                    ...
                    {/* === TAMPILAN BARU: STACKED === */}
                    <div className="mt-1">
                        {/* Netname (Bold & Gelap) */}
                        <p className="text-lg font-bold text-slate-800 leading-tight">
                            {netname}
                        </p>
                        {/* Nama PT (Lebih Kecil & Abu-abu) */}
                        {company && (
                        ...
                        {!company && netname !== data.holder && (
                             <p className="text-sm text-slate-500 font-medium">{data.holder}</p>
                        )}
                    </div>
