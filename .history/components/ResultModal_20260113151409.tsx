...
export default function ResultModal({ data, onClose, isHostmaster }: ResultModalProps) {
  ...
  // Helper untuk parsing Sub-allocation string biar rapi
  const parseChildString = (childStr: string) => {
    // Format backend: "Netname (Range) : Descr"
    const match = childStr.match(/^(.*?) \((.*?)\)(?: : (.*))?$/);
    if (match) {
      return { netname: match[1], range: match[2], desc: match[3] || "-" };
    }
  ...
  const handleExportCSV = () => {
    ...
    const rows = data.map(item => {
      ...
      // Ganti separator pipe | jadi newline biar rapi di excel
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                     ...
                     <div className="space-y-4">
                        ...
                        <div className="pl-3 border-l-2 border-blue-200 space-y-1">
                            ...
                            <p className="font-semibold text-slate-800 text-sm break-all">{item.parent_name}</p>
                            <p className="text-xs text-slate-500">{item.parent_net}</p>
                        </div>

                        {/* Sub-Allocations (YANG TADI HILANG) */}
                            ...
                            {item.children && item.children !== "-" ? (
                                <div className="space-y-2">
                                    {item.children.split(' | ').map((child, cIdx) => {
                                        ...
                                        return (
                                            <div key={cIdx} className="bg-slate-50 p-2 rounded border border-slate-100 text-xs">
                                                <div className="font-bold text-slate-700">{parsed.netname}</div>
                                                <div className="font-mono text-slate-500 text-[10px]">{parsed.range}</div>
                                            </div>
                                        )
                                    })}
                                </div>
