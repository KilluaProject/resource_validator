...
export default function ResultModal({ data, onClose }: ResultModalProps) {
  ...
  // Logic Max Length /24 (Tetep kita pertahanin karena ini penting)
  const getMaxLength = (cidr: string) => {
    if (cidr.includes(":")) return "48"; 
    return "24"; 
  };
    ...
    <div className="fixed inset-0 bg-slate-900/70 z-50 flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in duration-200">
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col border border-slate-200 overflow-hidden">
        ...
        <div className="px-8 py-6 border-b border-slate-100 bg-white flex justify-between items-start z-10">
          ...
          <button onClick={onClose} className="group bg-slate-50 hover:bg-red-50 p-2 rounded-xl border border-slate-200 hover:border-red-200 transition-all">
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-5">
                    ...
                    {/* Reverse DNS (Versi Rapi / Stabil) */}
                    <div>
                      <p className="text-[10px] text-slate-500 uppercase mb-1">Reverse DNS (PTR)</p>
                      <div className="bg-slate-800 text-slate-200 p-3 rounded-lg font-mono text-xs shadow-inner break-words">
                        {item.ptr_record}
                      </div>
                    </div>
                  ...
                  <div className="mt-8 relative group">
                    <div className="bg-slate-900 rounded-xl overflow-hidden shadow-lg border border-slate-700">
                      <div className="bg-slate-800 px-4 py-2 flex items-center justify-between border-b border-slate-700">
                         ...
                         <span className="text-[10px] font-mono text-slate-400 uppercase tracking-widest">Config_Suggestion.sh</span>
                      </div>
                      <button 
                        onClick={() => handleCopy(`ASN: ${getSuggestedASN(item.irr_objects)}\nPrefix: ${item.cidr}\nMax Length: /${getMaxLength(item.cidr)}`)}
                        className="absolute top-12 right-4 text-xs bg-white/10 hover:bg-white/20 text-slate-300 px-3 py-1 rounded transition border border-white/5 backdrop-blur-sm"
                      >
                        📋 Copy
                      </button>
        ...
        <div className="p-5 border-t border-slate-100 bg-white z-10 flex justify-end">
          <button onClick={onClose} className="px-8 py-2.5 bg-slate-800 hover:bg-slate-900 text-white rounded-xl text-sm font-bold shadow-lg shadow-slate-300/50 transition-all hover:-translate-y-0.5">
            Selesai
          </button>
        </div>
      </div>
