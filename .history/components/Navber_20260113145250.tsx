...
import LoginModal from "@/components/LoginModal"; // Import Modal disini

export default function Navbar() {
  const { isHostmaster, logout, login } = useAuth(); // Ambil data dari Global
  const [scrolled, setScrolled] = useState(false);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [showLoginModal, setShowLoginModal] = useState(false); // State Modal pindah kesini
  ...
  const handleLoginSuccess = () => {
    login(); // Panggil fungsi login global
    alert("🔓 Hostmaster Mode Unlocked!");
  };

  return (
    <>
      {/* Modal Login dipasang di Navbar biar available global */}
      <LoginModal 
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            <Link href="/" className="flex-shrink-0 flex items-center gap-2 cursor-pointer group">
              ...
              <span className={`font-extrabold text-xl tracking-tight transition-colors ${scrolled ? "text-slate-900" : "text-slate-800"}`}>
        ...
        {/* MOBILE MENU */}
        {mobileMenuOpen && (
          <div className="md:hidden bg-white/95 backdrop-blur-xl border-t border-slate-100 px-4 py-4 space-y-3 shadow-xl absolute w-full">
