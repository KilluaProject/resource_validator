...
export default function ResultModal({ data, onClose }: ResultModalProps) {
  ...
  const handleExportCSV = () => {
    ...
    const rows = data.map(item => {
      // Bersihin data biar aman di CSV (hapus enter, ganti koma jadi spasi)
      const clean = (text: string) => `"${text.replace(/"/g, '""')}"`; // Wrap pake tanda kutip
