...
export default function ResultModal({ data, onClose }: ResultModalProps) {
  ...
  const handleExportCSV = () => {
    // 1. UPDATE HEADER: Tambah kolom "Sub-Allocations"
    const headers = [
      "CIDR", "Parent Network", "Parent Name", "Parent Description", "Sub-Allocations", 
      "RPKI Status", "RPKI Detail", 
    ...
    const rows = data.map(item => {
      ...
      // 2. FORMAT SUB-ALLOCATION: Ganti pemisah " | " jadi Enter (\n) biar rapi di Excel
      // Kalau kosong, isi "-"
      const subAllocations = item.children && item.children !== "-" 
        ? item.children.replace(/ \| /g, "\n") 
        : "-";
      ...
      return [
        ...
        clean(item.parent_desc),
        clean(subAllocations), // <--- DATA BARU DISINI
