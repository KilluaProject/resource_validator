...
# ==========================================
# 4. CONTROLLER LOGIC (UPDATED WITH DESCR)
# ==========================================
def scan_ip_logic(cidr_str):
    ...
    if unique_hierarchy:
        for obj in unique_hierarchy[1:]:
            # UPDATE: Ambil description anak-anaknya juga
            c_desc = obj.get('descr', '').replace(" | ", ", ") # Gabungin kalo multi-line
            # Format baru: NETNAME (RANGE) : DESCR
            child_str = f"{obj.get('netname', '?')} ({obj.get('_range')})"
            if c_desc and c_desc != "-":
                # Potong dikit kalo kepanjangan biar gak ngerusak UI
    ...
    return {
        ...
        "parent_net": p_net,
        "parent_name": p_name,
        "parent_desc": p_desc, # Sudah rapi (comma separated)
        "children": children_str, # Sudah ada descriptionnya
