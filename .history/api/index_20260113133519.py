...
# ==========================================
# 0. SECURITY VALIDATOR (SATPOL PP BACKEND)
# ==========================================
def is_safe_input(text, type="IP"):
    """
    Validasi ketat format input di sisi server.
    Mencegah injection atau input sampah yang lolos dari frontend.
    """
    if len(text) > 50: return False # Mencegah Buffer Overflow sederhana

    if type == "ASN":
        # Harus diawali AS, diikuti angka. Case insensitive.
        return bool(re.match(r'^AS\d+$', text, re.IGNORECASE))
    else:
        # Cek IPv4 CIDR / Single IP (Accept x.x.x.x or x.x.x.x/xx)
        ipv4_pattern = r'^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(?:\/(?:3[0-2]|[1-2]?[0-9]))?$'
        
        # Cek IPv6 (Minimal check hex dan colon)
...
def task_apnic_hierarchy(cidr):
    ...
    unique_hierarchy = []
    seen = set()
    IGNORE_NAMES = ['APNIC-AP', 'IANA', 'ERX-NETBLOCK', 'ARIN-CIDR-BLOCK', 'ADMINISTERED-BY-RIPE', 'AFRINIC-CIDR-BLOCK', 'LACNIC-CIDR-BLOCK']
    ...
    for obj in hierarchy_list:
        ...
        # === FILTER: IDNIC SOURCE SKIP ===
        if 'IDNIC' in source: continue 
...
async def scan_endpoint(payload: InputData):
    ...
    # === [SECURITY] 1. LIMIT BARIS (DoS Prevention) ===
    if len(raw_lines) > 10:
        raise HTTPException(status_code=400, detail="Too many lines. Maximum 10 IPs allowed.")
    ...
    for line in raw_lines:
        ...
        # === [SECURITY] 2. VALIDASI INPUT (Regex/Safe Input) ===
        if not is_safe_input(line, type="IP"):
             # Kita skip yang aneh, dan print ke console backend untuk log
             print(f"Warning: Malformed input detected and skipped: {line}")
             continue
...
async def asn_endpoint(payload: ASNInput):
    ...
    # === [SECURITY] VALIDASI ASN ===
    if not is_safe_input(asn_clean, type="ASN"):
         raise HTTPException(status_code=400, detail="Invalid ASN format. Use ASxxxxx")
