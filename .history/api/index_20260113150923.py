...
# ==========================================
# 2. WORKER TASKS (CORE LOGIC)
# ==========================================

# --- TASK A: APNIC HIERARCHY (FIXED: NO FILTER, ADD TAGGING) ---
def task_apnic_hierarchy(cidr):
    ...
    for obj in hierarchy_list:
        ...
        # === 1. JANGAN DI-SKIP (FILTER DIHAPUS) ===
        # Kita ijinkan semua source masuk dulu.
        
        # === 2. TAGGING SOURCE IDNIC ===
        # Kalau source-nya dari IDNIC, kita kasih tanda di nama-nya.
        # Kita pake kurung SIKU [] biar gak ngerusak regex parser Frontend.
        if 'IDNIC' in source:
            current_netname = obj.get('netname', '')
            obj['netname'] = f"{current_netname} [@source IDNIC]"
...
async def asn_endpoint(payload: ASNInput):
    ...
    if not is_safe_input(asn_clean, type="ASN"):
         raise HTTPException(status_code=400, detail="Invalid ASN format.")
