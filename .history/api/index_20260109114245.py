...
# ==========================================
# 3. MODUL REVERSE DOMAIN & DELEGATION
# ==========================================
...
def scan_ip_logic(cidr_str):
    ...
    if unique_hierarchy:
        for obj in unique_hierarchy[1:]:
            ...
            if c_desc and c_desc != "-":
                ...
                child_str += f" : {c_desc}"
            children_txt_list.append(child_str)
    ...
    # --- PHASE 2: IRR & VISIBILITY (RIPEstat + RADB) ---
    final_irr_list = route_list[:] # Mulai dari data APNIC Whois
    
    rpki_status, rpki_detail, visibility = "UNKNOWN", "-", "Not Seen"
    try:
        # A. RIPEstat (Global Aggregator)
        ...
        # B. RADB Direct Check (NEW!)
        # Kita tarik langsung dari whois.radb.net buat mastiin
        radb_objects = get_radb_irr_objects(cidr_str)
        for obj in radb_objects:
            if obj not in final_irr_list:
                final_irr_list.append(obj)
