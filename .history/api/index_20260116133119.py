...
def task_apnic_hierarchy(cidr):
    ...
    for obj in hierarchy_list:
        ...
        if size >= 16000000: continue # Skip /8 ke atas
        
        seen.add(rng)
        obj['_size'] = size
        temp_list.append(obj)

    # === BAGIAN YANG DIHAPUS: LOGIC "IS_SELF_PORTABLE" ===
    # Dulu di sini ada filter yang nge-drop Parent kalau Child-nya Portable.
    # Sekarang kita hapus, jadi SEMUA hierarchy yang valid bakal masuk.
    # Parent terbesar otomatis akan naik ke paling atas setelah di-sort.
    
    unique_hierarchy = temp_list
    
    # Sort by Size (Biggest First = Parent)
    ...
    if len(unique_hierarchy) > 1:
        # Mulai dari index 1 karena index 0 adalah Parent
