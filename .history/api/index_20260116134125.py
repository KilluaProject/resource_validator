...
def task_apnic_hierarchy(cidr):
    ...
    for obj in hierarchy_list:
        ...
        # Tagging IDNIC (Biar user tau ini data dari mana)
        if 'IDNIC' in source:
            name_ori = obj.get('netname', '')
            obj['netname'] = f"{name_ori} [@source IDNIC]"

        # === DEDUPLIKASI CERDAS ===
        # Kita pakai kunci: "RANGE + SOURCE"
        # Jadi kalau Range sama TAPI Source beda (misal satu APNIC, satu IDNIC),
        # dua-duanya tetep lolos masuk.
    ...
    # === SORTING PRIORITAS ===
    # 1. Size Terbesar di atas (Parent harus paling atas).
    # 2. Kalau Size sama, Source APNIC (polos) di atas Source IDNIC.
    def sort_priority(x):
        # Kalau ada 'IDNIC' di source, score priority 0. Kalau gak ada (APNIC), score 1.
        # Karena reverse=True, yang score-nya 1 (APNIC) bakal di atas.
    ...
    if len(unique_hierarchy) > 1:
        # Mulai dari index 1 karena index 0 dipakai sebagai Parent
