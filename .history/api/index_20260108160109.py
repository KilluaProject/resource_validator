...
def get_whois_delegation(cidr_obj):
    # Fungsi ini ngecek 'nserver' di Whois APNIC
    zone_name = get_reverse_zone(cidr_obj)
    if not zone_name:
        return None
    
    # Query domain object (-rB biar gak recursive berlebihan, domain object doang)
...
# ==========================================
# 4. CONTROLLER LOGIC (UPDATED WITH SMART DELEGATION)
# ==========================================
def scan_ip_logic(cidr_str):
    ...
    # --- PHASE 3: REVERSE DNS & WHOIS DELEGATION (SMART CHECK) ---
    final_ptr_display = ""
    
    try:
        net = ipaddress.ip_network(cidr_str, strict=False)
        
        # LOGIC BARU: Kalo IPv4 dan prefix < 24 (misal /23), kita cek per /24
        if net.version == 4 and net.prefixlen < 24:
            results = []
            results.append("Detected Aggregate Block (Checking /24 Delegation):")
            
            # Loop setiap /24 di dalam blok ini
            for subnet in net.subnets(new_prefix=24):
                nservers = get_whois_delegation(subnet)
                if nservers:
                    # Kalo ada nserver (delegasi myAPNIC)
                    ns_str = ", ".join(nservers[:2]) # Ambil 2 biji aja
            ...
            # LOGIC LAMA: Buat /24 atau /32 (Single Check)
            ...
            # 2. Cek Whois Delegation
            whois_ns = ""
            nservers = get_whois_delegation(net)
            if nservers:
                whois_ns = f"Delegated to: {', '.join(nservers[:2])}"
            
            # 3. Tentuin mana yg mau ditampilin
            if "No DNS" not in dns_ptr:
                final_ptr_display = f"{dns_ptr} (Live DNS)"
            elif whois_ns:
                final_ptr_display = f"{whois_ns} (APNIC Whois)"
            else:
                final_ptr_display = "No Reverse DNS & No Delegation"
