...
# --- TASK A: APNIC HIERARCHY (FIXED: IGNORE ERX & ROOT BLOCKS) ---
def task_apnic_hierarchy(cidr):
    ...
    # === FILTER LIST: BLOK RAKSASA YG WAJIB DI-SKIP ===
    IGNORE_NAMES = [
        'APNIC-AP', 
        'IANA', 
        'ERX-NETBLOCK', 
        'ARIN-CIDR-BLOCK', 
        'ADMINISTERED-BY-RIPE',
        'AFRINIC-CIDR-BLOCK',
        'LACNIC-CIDR-BLOCK'
    ]
    ...
    for obj in hierarchy_list:
        ...
        # 2. Skip by Name (Blacklist)
        if any(bad in name for bad in IGNORE_NAMES):
            continue

        # 3. Skip by Size (Anti /8)
        # 16,000,000 IP itu pasti Root Registry, bukan Parent yang lu cari
    ...
    # Fallback: Kalau abis difilter malah kosong, terpaksa ambil Self
    if not parent_obj:
...
async def scan_endpoint(payload: InputData):
    ...
    for cidr in valid_cidrs:
        try:
            ...
            results.append(data)
            time.sleep(0.5) # Safety Sleep (Anti-Ban)
        except Exception as e:
