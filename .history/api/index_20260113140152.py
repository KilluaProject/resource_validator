...
# ==========================================
# 0. SECURITY VALIDATOR (SATPAM BACKEND)
# ==========================================
...
def task_apnic_hierarchy(cidr):
    ...
    for obj in hierarchy_list:
        ...
        # Kita skip data yang source-nya IDNIC biar gak duplikat/bingung
        if 'IDNIC' in source: 
            continue 
    ...
    for obj in temp_list:
        if obj.get('_range') == input_range_v4:
            ...
            if 'PORTABLE' in status:
                is_self_portable = True
                break
    
    # 4. Final Processing
    for obj in temp_list:
        if is_self_portable and obj['_size'] > self_size:
            continue
...
async def scan_endpoint(payload: InputData):
    ...
    if len(raw_lines) > 50:
        raise HTTPException(status_code=400, detail="Too many lines. Maximum 50 IPs allowed.")
    ...
    for line in raw_lines:
        ...
        if not is_safe_input(line, type="IP"):
             print(f"Warning: Malformed input detected: {line}")
             continue
