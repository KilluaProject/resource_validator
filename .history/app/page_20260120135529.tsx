...
import ASNResultModal from "@/components/ASNResultModal"; // <--- Import Component Baru
...
export default function Home() {
  ...
  // State Modal
  const [showModal, setShowModal] = useState(false);       // Buat IP
  const [showASNModal, setShowASNModal] = useState(false); // Buat ASN (BARU)
  ...
  return (
    <main className="min-h-screen bg-slate-50 relative overflow-hidden pb-20">
      ...
      {/* RENDER MODAL ASN (BARU) */}
      {showASNModal && asnResult && (
        <ASNResultModal data={asnResult} onClose={() => setShowASNModal(false)} />
      )}
    </main>
  );
}