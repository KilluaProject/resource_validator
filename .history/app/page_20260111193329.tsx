...
export default function Home() {
  ...
  const handleScanIP = async (targets: string) => {
    ...
    if(mode === "IP") { 
        setAsnResult(null); 
        setResults([]); // Reset hasil dulu
        setShowModal(false); // Pastikan modal nutup dulu pas loading
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        try {
            ...
            // KITA GAK UPDATE STATE 'results' DISINI LAGI
            // Biar UI gak 'kaget' update satu-satu
            accumulatedResults = [...accumulatedResults, ...data];
            
        } catch (error) { console.error(error); }
        
        // Update Progress Bar di Tombol
        setProgress(i + 1);
    }
    
    // === SETELAH SEMUA SELESAI ===
    setResults(accumulatedResults); // Masukin semua data sekaligus
    saveToHistory("IP", targets, accumulatedResults, undefined);
    
    setLoading(false);
    setShowModal(true); // BARU DIBUKA MODALNYA DISINI
  };
