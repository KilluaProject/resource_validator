...
export default function Home() {
  ...
  // === SECURITY: VALIDATION HELPER (SATPAM FRONTEND) ===
  const validateInput = (text: string, type: "IP" | "ASN") => {
    const cleanText = text.trim();
    
    if (type === "ASN") {
        // Harus diawali AS dan diikuti angka. Contoh: AS136115 (Case Insensitive)
        return /^AS\d+$/i.test(cleanText);
    } else {
        // Regex IPv4 CIDR (Standard)
        const ipv4Regex = /^(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(?:\/(?:3[0-2]|[1-2]?[0-9]))?$/;
        
        // Regex IPv6 Simple (Cek karakter hex dan colon)
  ...
  const handleScanIP = async (targets: string) => {
    ...
    // 2. VALIDATION CHECK (Baris per Baris)
    for (let i = 0; i < lines.length; i++) {
        if (!validateInput(lines[i], "IP")) {
            alert(`⛔ Format Salah di Baris ke-${i+1}: "${lines[i]}"\n\nPastikan formatnya IP Address (IPv4/IPv6) atau CIDR yang benar.\nContoh: 103.10.10.0/24`);
            return; // STOP PROSES JIKA ADA YANG SALAH
        }
    }
  ...
  const handleScanASN = async () => {
    ...
    // VALIDATION CHECK ASN
    if (!validateInput(input, "ASN")) {
        alert(`⛔ Format ASN Salah: "${input}"\n\nHarus diawali dengan "AS" diikuti angka.\nContoh: AS136115`);
        return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (data.detail) { // Handle FastAPI error
