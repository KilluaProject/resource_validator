...
export default function Home() {
  ...
  const handleLoginSuccess = () => {
    ...
    alert("🔓 Hostmaster Mode Unlocked: Bulk Check & CSV Active!");
  };
  ...
  const validateInput = (text: string, type: "IP" | "ASN") => {
    ...
    return ipv4Regex.test(cleanText) || ipv6Regex.test(cleanText);
  };
  ...
  const handleScanASN = async () => {
    if (!input) return;
    if (!validateInput(input, "ASN")) {
        alert(`⛔ Format ASN Salah. Contoh: AS136115`);
        return;
    }
    setLoading(true);
    setAsnResult(null);
  ...
  return (
    <main className="min-h-screen bg-slate-50 relative overflow-hidden pb-20">
      ...
      <div className="absolute top-4 right-4 z-50">
          {isHostmaster ? (
              <button onClick={handleLogout} className="flex items-center gap-2 bg-slate-900 text-white px-4 py-2 rounded-full text-xs font-bold shadow-lg hover:bg-red-600 transition-all">
                  <span>👮 Hostmaster Mode</span>
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg>
              </button>
          ) : (
              <button onClick={() => setShowLogin(true)} className="text-slate-400 hover:text-slate-800 transition-colors p-2" title="Hostmaster Login">
      ...
      {showModal && results.length > 0 && (<ResultModal data={results} onClose={() => setShowModal(false)} isHostmaster={isHostmaster} />)}
    </main>
  );
}